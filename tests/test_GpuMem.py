...
pixel_formats=[[ k,v[0]] for k, v in nvc.PixelFormat.__entries.items() if k!='UNDEFINED' ]

def dlpackptr(plane:nvc.SurfacePlane):
...
class TestGpuMem(unittest.TestCase):
    ...
    '''@parameterized.expand([
        ["nv12", nvc.PixelFormat.NV12,],
        ["rgb", nvc.PixelFormat.RGB,],
    ])'''
    @ parameterized.expand(pixel_formats)
    def test_gpu_mem(self, name, pix_fmt):
        surf = nvc.Surface.Make(format = pix_fmt, width = 640, height = 480, gpu_id = 0)
