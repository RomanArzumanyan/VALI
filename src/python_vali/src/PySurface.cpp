...
void Init_PySurface(py::module& m) {
  ...
  py::class_<Surface, shared_ptr<Surface>>(
      ...
      .def(py::init([](const PyDecoder& decoder, optional<Pixel_Format> format, int gpuID) {
        ...
        py::arg("decoder"), py::arg("format") = std::nullopt, 
        py::arg("gpu_id") = 0, py::return_value_policy::take_ownership,
        R"pbdoc(
        Constructor.

        Initializes the Surface using properties from a PyDecoder object.
        If the `format` argument is provided, it overrides the format from the PyDecoder.

        :param decoder: PyDecoder object from which to initialize the Surface.
        :param format: (Optional) target pixel format. If not provided, uses decoder's format.
        :param gpu_id: GPU ID to use.
    )pbdoc")    
      .def(py::init([](const PyNvEncoder& encoder, optional<Pixel_Format> format, int gpuID) {
            return std::shared_ptr<Surface>(
                Surface::Make(format.has_value() ? format.value() : encoder.GetPixelFormat(), 
                              encoder.Width(), encoder.Height(),
                              CudaResMgr::Instance().GetCtx(gpuID)));
        }),
        py::arg("encoder"), py::arg("format") = std::nullopt, 
        py::arg("gpu_id") = 0, py::return_value_policy::take_ownership,
        R"pbdoc(
        Constructor.

        Initializes the Surface using properties from a PyNvEncoder object.
        If the `format` argument is provided, it overrides the format from the PyNvEncoder.

        :param encoder: PyNvEncoder object from which to initialize the Surface.
        :param format: (Optional) target pixel format. If not provided, uses encoder's format.
        :param gpu_id: GPU ID to use.
    )pbdoc")    
