...
PYBIND11_MODULE(_python_vali, m) {
  ...
  py::class_<MotionVector, std::shared_ptr<MotionVector>>(
      ...
      .def_readwrite("source", &MotionVector::source,
                     "Frame reference direction indicator. Negative value "
                     "indicates past reference, "
      ...
      .def_readwrite("src_x", &MotionVector::src_x,
                     "Absolute X coordinate of the source macroblock. May be "
                     "outside frame boundaries.")
      .def_readwrite("src_y", &MotionVector::src_y,
                     "Absolute Y coordinate of the source macroblock. May be "
                     "outside frame boundaries.")
      .def_readwrite("dst_x", &MotionVector::dst_x,
                     "Absolute X coordinate of the destination macroblock. May "
                     "be outside frame boundaries.")
      .def_readwrite("dst_y", &MotionVector::dst_y,
                     "Absolute Y coordinate of the destination macroblock. May "
                     "be outside frame boundaries.")
      ...
      .def_readwrite("motion_scale", &MotionVector::motion_scale,
                     "Motion vector precision factor. For example, 4 indicates "
  ...
  py::class_<SeekContext, shared_ptr<SeekContext>>(
      m, "SeekContext", "Context object for video frame seeking operations.")
  ...
  py::class_<PacketData, shared_ptr<PacketData>>(
      m, "PacketData", "Video frame metadata container")
      .def(py::init<>())
      .def_readwrite("key", &PacketData::key,
                     "Frame type indicator. 1 for I-frames (key frames), 0 for "
                     "other frames.")
  ...
  py::class_<CudaStreamEvent, shared_ptr<CudaStreamEvent>>(
      m, "CudaStreamEvent", "CUDA stream synchronization event")
  ...
  py::class_<TaskExecDetails, shared_ptr<TaskExecDetails>>(
      m, "TaskExecDetails",
  ...
  py::class_<StreamParams, shared_ptr<StreamParams>>(
      m, "StreamParams", "Video stream parameters container")
      .def(py::init<>())
      ...
      .def_readwrite("level", &StreamParams::level, "Codec level identifier")
      .def_readwrite("codec_id", &StreamParams::codec_id, "Codec identifier")
