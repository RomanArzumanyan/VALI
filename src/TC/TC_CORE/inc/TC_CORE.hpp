...
enum class TaskExecInfo {
  ...
  NOT_SUPPORTED,
  RES_CHANGE,
  SRC_DST_SIZE_MISMATCH,
  SRC_DST_FMT_MISMATCH
};

struct TaskExecDetails {
  TaskExecStatus m_status = TaskExecStatus::TASK_EXEC_SUCCESS;
  TaskExecInfo m_info = TaskExecInfo::SUCCESS;
  std::string m_msg;

  TaskExecDetails() = default;

  TaskExecDetails(TaskExecStatus const& status, TaskExecInfo const& info)
      : m_status(status), m_info(info) {}

  TaskExecDetails(TaskExecStatus const& status, TaskExecInfo const& info,
                  std::string const& msg)
      : m_status(status), m_info(info), m_msg(msg) {}
};
...
public:
   ...
   */
  virtual TaskExecDetails Run();

  /* Call this method to run the task;
   */
  virtual TaskExecDetails Execute();

  /* Updates exec details;
   */
  void SetExecDetails(const TaskExecDetails& exec_details);

  /* Get last exec details;
   */
  void GetExecDetails(TaskExecDetails& exec_details);
