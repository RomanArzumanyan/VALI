...
bool FFmpegDemuxer::Demux(uint8_t *&pVideo, size_t &rVideoBytes,
  ...
  auto appendBytes = [](std::vector<uint8_t> &elementaryBytes,
                        AVPacket &avPacket, AVPacket &avPacketOut,
                        AVBSFContext *pAvbsfContext, int streamId,
    ...
    do {
      ...
      if (condition > 0) {
        if (tmp_ctx.IsByNumber()) {
          ...
          tmp_ctx.seek_tssec = std::max(0.0, tmp_ctx.seek_tssec);
        } else {
          details.info = TaskExecInfo::FAIL;
          throw std::runtime_error("Invalid seek mode.");
        }
...
AVFormatContext *FFmpegDemuxer::CreateFormatContext(
    const char *szFilePath,
    const std::map<std::string, std::string> &ffmpeg_options) {
  ...
  if (err < 0 || nullptr == ctx) {
    av_dict_free(&options);
    std::stringstream ss;
    ss << "Can't open " << szFilePath << ": " << AvErrorToString(err) << "\n";
    throw std::runtime_error(ss.str());
  }
...
FFmpegDemuxer::FFmpegDemuxer(AVFormatContext *fmtcx) : fmtc(fmtcx) {
  ...
  if (!fmtc) {
    std::stringstream ss;
    ss << __FUNCTION__ << ": no AVFormatContext provided.";
  ...
  if (!bfs_name.empty()) {
    ...
    if (0 != ret) {
      throw std::runtime_error("Error allocating " + bfs_name +
    ...
    if (0 != ret) {
      throw std::runtime_error("Error initializing " + bfs_name +
                               " bitstream filter: " + AvErrorToString(ret));
    }
  }
...
DemuxFrame::DemuxFrame(const char *url, const char **ffmpeg_options,
  ...
  if (0 == opts_size % 2) {
    for (auto i = 0; i < opts_size;) {
      auto key = std::string(ffmpeg_options[i]);
      i++;
      auto value = std::string(ffmpeg_options[i]);
      i++;
...
void DemuxFrame::GetParams(MuxingParams &params) const {
  ...
  default:
    ...
    ss << "Unsupported FFmpeg pixel format: "
       << av_get_pix_fmt_name(pImpl->demuxer->GetPixelFormat());
    throw std::invalid_argument(ss.str());
